...
# ANSWER: query not unmapped; mate not unmapped; not a secondary alignment; query aligns to plus strand; mate aligns to minus strand;
# fragment length between 0 and 1000; cut the length field out; go 1 million lines into file; extract 100 thousand lines, write to file. 
...
# Roughly, it is this is number of mapped reads that are primary alignments, and are not duplicates, times readlength, divided by genome size
...
# Physical coverage for the haploid genome
